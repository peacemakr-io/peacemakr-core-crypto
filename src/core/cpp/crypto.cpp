...
ciphertext_blob_t *
peacemakr::CryptoContext::Deserialize(const std::string &serialized, crypto_config_t *out_cfg) {
  if (serialized.empty()) {
    m_log_("noting to deserialize");
    return nullptr;
  }
  return peacemakr_deserialize((uint8_t *)serialized.data(), serialized.size(), out_cfg);
}
