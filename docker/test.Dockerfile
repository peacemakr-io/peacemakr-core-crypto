...
RUN apk add --no-cache libbsd-dev git alpine-sdk perl cmake linux-headers clang-analyzer clang-dev llvm-dev
...
RUN mkdir -p build && cd build && CC=clang CXX=clang++ cmake .. && make check

CMD test_fuzz /opt/CORPUS -max_len=16384 -jobs=4
